...
// Names resolved at runtime via Intl.DisplayNames so the list stays small.
...
const fallbackNames = {
  // Used only if Intl.DisplayNames isn't available; codes as labels.
};
...
export function getCountries(locale = 'en') {
  ...
  return CODES.map((code) => ({
    code,
    name: dn?.of(code) || fallbackNames[code] || code,
  })).sort((a, b) => a.name.localeCompare(b.name));
}

export const COUNTRY_CODES = CODES;

// ISO 3166-1 alpha-2 → ITU-T E.164 calling code (without leading "+")
